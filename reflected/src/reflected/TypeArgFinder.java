...
public class TypeArgFinder {
    ...
    public static Class<?> tryToClass(Type type) {
        if (type instanceof Class<?>) {
            ...
            Class<?> cc = tryToClass(((GenericArrayType) type).getGenericComponentType());
            if (cc != null) {
    ...
    private static UnsupportedOperationException buildException(Class<?> targetClass, Class<?> parameterizedClass,
            int index) {
        return new UnsupportedOperationException("无法确定范型类的第" + index + "个范型参数");
    }

    public static Class<?>[] get(Class<?> targetClass, Class<?> parameterizedClass) {
        ParameterizedType[] paramTypes = new ParameterizedType[8];
        int paramTypeCount = 0;
        for (; targetClass != parameterizedClass; targetClass = targetClass.getSuperclass()) {
            ...
            if (t instanceof ParameterizedType) {
                if (paramTypeCount == paramTypes.length) {
                    ...
                    System.arraycopy(paramTypes, 0, newParamTypes, 0, paramTypeCount);
        ...
        for (;;) {
            ...
            for (int i = 0; i < typeArgIndexes.length; i++) {
                ...
                if (t instanceof Class<?>) {
                    ...
                    if (tvs == null) {
                        tvs = ((TypeVariable<?>) t).getGenericDeclaration().getTypeParameters();
                    }
    ...
    public static Class<?> find(Class<?> targetClass, Class<?> parameterizedClass, int index) {
        return findOrGet(targetClass, parameterizedClass, index, true);
    }

    public static Class<?> get(Class<?> targetClass, Class<?> parameterizedClass, int index) {
        return findOrGet(targetClass, parameterizedClass, index, false);
    }

    private static Class<?> findOrGet(Class<?> targetClass, Class<?> parameterizedClass, int index, boolean find) {
        ParameterizedType[] paramTypes = new ParameterizedType[8];
        int paramTypeCount = 0;
        ...
        for (;;) {
            ...
            if (t instanceof Class<?>) {
                ...
                TypeVariable<?>[] tvs = ((TypeVariable<?>) t).getGenericDeclaration().getTypeParameters();
                for (int j = 0; j < tvs.length; j++) {
                    if (tvs[j] == t) {
                        typeArgIndexe = j;
                        break;
    ...
    public static ParameterizedType findParameterizedType(Class<?> targetClass, Class<?> parameterizedClass,
            int index) {
        ParameterizedType[] paramTypes = new ParameterizedType[8];
        int paramTypeCount = 0;
        if (parameterizedClass.isInterface()) {
    ...
    @Deprecated
    public static Class<?> find(Class<?> ownerClass, ParameterizedType fieldType, int index) {
