...
public class RedisCacheImpl2 implements ICache {
    private static final Logger LOGGER = Logger.getLogger(RedisCacheImpl2.class);
    ...
    static {
        ...
        for (int i = 0; i < shareds.size(); i++) {
            jedisClusterNodes.add(new HostAndPort(shareds.get(i).getIp(), shareds.get(i).getPort()));
        }
        // 构造池
    ...
    public void addCacheData(Date loadDate, String queryArgs, Object data) {
        if (ISCACHEON) {
            ...
            cluster.setex(queryArgs.getBytes(), keepAliveTime, this.getStrFromObj(data));
        } else {
    ...
    public void addCacheData(String queryArgs, CacheData data) {
        if (ISCACHEON) {
            LOGGER.debug("添加缓存：参数" + queryArgs);
            cluster.setex(queryArgs.getBytes(), keepAliveTime, this.getStrFromObj(data.getData()));
        } else {
            LOGGER.debug("配置禁用缓存" + queryArgs);
        }
    }
    ...
    public void addGlobleCacheData(String queryArgs, Object data) {
        if (ISCACHEON) {
            ...
            cluster.set(queryArgs.getBytes(), this.getStrFromObj(data));
        } else {
            LOGGER.debug("配置禁用缓存" + queryArgs);
        }
    }
