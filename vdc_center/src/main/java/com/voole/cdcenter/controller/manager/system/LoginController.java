...
public class LoginController extends BaseController {
	...
	public WebAsyncTask<String> login(String username, String passwordcode, HttpSession session, Model mpdel) {
		return new WebAsyncTask<>(() -> {
			...
			try {
				...
				List<UserVo> loginUserList = LoginController.this.userService.queryUser(userqc);
				...
				if (null != loginUser) {
					...
					if (pw.equals(loginUser.getPasswordcode())) {
						...
						// 重置登录次数
						LoginController.this.userService.updateUser(loginUser);
						RuleVo rulequerycase = new RuleVo();
						rulequerycase.setUid(loginUser.getUid());
						List<RuleVo> rules = LoginController.this.ruleService.queryUserRules(rulequerycase);
						...
						List<FunctionVo> functions = LoginController.this.functionService
								.queryMenuFunction(functionquerycase);
						FunctionVo allfunctionquerycase = new FunctionVo();
						allfunctionquerycase.setRids(rids);
						allfunctionquerycase.setIs_on(1);
						List<FunctionVo> allFunctions = LoginController.this.functionService
								.queryFunction(allfunctionquerycase);
				...
				LoginController.this.LOGGER.error(e.getMessage(), e);
			}
			return "login";
		});
	}
	...
	@ResponseBody
	public WebAsyncTask<AjaxRet> unlock(String passwordcode, HttpSession session) {
		return new WebAsyncTask<>(() -> {
	...
	public WebAsyncTask<String> loginout(HttpServletRequest request) {
		return new WebAsyncTask<>(() -> {
